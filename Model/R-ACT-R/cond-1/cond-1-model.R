...
#-------------------------------------------------
#
# Model of Stone_et_al (2021) Experiment-2 MATCH condition
# Sentence ['match' condition]: Klicke auf seinen blauen Knopf
#
# Processing steps: 
# "Klicke_auf" "seinen" <ANTECEDENT RETRIEVAL> <PICTURE RETRIEVAL_1> "blauen" <PICTURE RETRIEVAL_2> "Knopf" <PICTURE RETRIEVAL_3>
# 
# 
# . ALGORITHM for parsing the sentence
#   0. '<empty>' 
#        - RETRIEVE: none
#        - CREATE: an empty IP chunk
#   1. 'klick_auf'
#        - RETRIEVE: an IP chunk
#        - CREATE: a VP chunk (with name="klicke_auf" and spec="t_s")
#   2. 'seinen'
#        - RETRIEVE: a VP chunk
#        - CREATE: a DP-empty chunk (with name="empty-dp", cat="DP-empty", spec="seinen", gend="masc.PRED", case="acc.PRED")
#          . Note: (i) there is no separate XP for 'seinen' [following Sol's syntax tree], 
#                  (ii) the values for 'gend' and 'case' have '.PRED' because these are only predicted XPs and should not cause retrieval interference
#        - CREATE: an NP-empty chunk (with name="empty-np", cat="NP-empty", gend="masc.PRED", case="acc.PRED")
#   3. 'blauen'
#        - RETRIEVE: an NP-empty chunk (with cat="NP-empty", gend="masc.PRED", case="acc.PRED")
#        - CREATE: an AdjP chunk (with cat="AdjP", name="blauen", gend="masc", case="acc")
#   4. 'Knopf'
#        - RETRIEVE: a DP chunk (with cat="DP", gend="masc", name="knopf", col="blau")
#          . Note: 
#            - the retrieval is of a DP instead of the 'NP-empty' because it's almost an antecedent resolution at 'Knopf' having already created a chunk for the picture 'Knopf'
#            - also the 'case' cue is not used because that's irrelevant for antecedent resolution
#            - maybe the 'name' cue should be weighted higher just like the 'color' cue because it clearly distinguishes the target
#            - and we assume that there is a process that links the retrieved DP to the empty-np as it's "antecedent" / referent
# 
# 
...
possPro.antc     = list(type="syn-obj", cat="DP", gend="masc", animacy="ani")
